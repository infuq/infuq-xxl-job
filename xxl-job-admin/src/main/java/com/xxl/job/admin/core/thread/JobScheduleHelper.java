    ...
    public void start(){
        ...
        scheduleThread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    // 表示scheduleThread线程只选择在一分钟内的第0,5,10,15,20,25,30,35,40,45,50,55秒被执行
                ...
                // pre-read count: treadpool-size * trigger-qps (each trigger cost 50ms, qps = 1000/50 = 20)      =            (200 + 100) * 20 = 6000,每次默认读取6000个任务
                ...
                while (!scheduleThreadToStop) {
                    ...
                    try {
                        ...
                        // 查询触发时间小于等于 (nowTime + 5s) 的任务,最多查询preReadCount条记录
                        List<XxlJobInfo> scheduleList = XxlJobAdminConfig.getAdminConfig().getXxlJobInfoDao().scheduleJobQuery(nowTime + PRE_READ_MS, preReadCount);
                        if (scheduleList!=null && scheduleList.size()>0) {
                            // 2、push time-ring
                            for (XxlJobInfo jobInfo: scheduleList) {

                                // time-ring jump  错过调度的场景
                                if (nowTime > jobInfo.getTriggerNextTime() + PRE_READ_MS) {

                                    /*
                                     *
                                     * ------[taskTime]------[taskTime+5s]--------[current]----------[current+5s]------------->
                                     *           |                                   |
                                     *       任务触发时间                           当前时间
                                     */
